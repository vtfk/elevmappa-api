...
module.exports = async (fnr, userId, studentUsername) => {
  ...
  await Promise.all(config.P360.map(async (source) => {
    ...
    try {
      ...
      if (!documents) {
        logger('info', ['retrieve-documents', userId, studentUsername, source.name, 'no documents found'])
        return
      }

      logger('info', ['retrieve-documents', userId, studentUsername, source.name, 'documents', documents.length])
