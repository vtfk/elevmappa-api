...
module.exports.getStudent = async (request, response) => {
  ...
  try {
    ...
    try {
      const resultMongo = await mongoLogger(username, id, `${username}-${id}`, `${id}`, `${username} opened ${id}'s archive folder`)
      logger('info', ['getStudent', username, id, 'log to mongo', resultMongo])
    } catch (error) {
...
module.exports.getFile = async (request, response) => {
  ...
  try {
    ...
    try {
      const resultMongo = await mongoLogger(username, studentId, `${studentId}`, `${username} viewed file with ID ${documentId} and recno ${recno} from source ${source}!`)
      logger('info', ['getFile', username, studentId, 'log to mongo', resultMongo])
    } catch (error) {
