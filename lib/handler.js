...
module.exports.getStudent = async (request, response) => {
  ...
  try {
    ...
    try {
      const result = await mongoLogger(username, id, `${username} opened ${id}'s archive folder`)
      logger('info', ['getStudent', username, id, 'log to mongo', result.insertedCount])
    } catch (error) {
...
module.exports.getFile = async (request, response) => {
  ...
  try {
    ...
    try {
      const result = await mongoLogger(username, studentId, `${username} viewed file with ID ${documentId} and recno ${recno} from source ${source}!`)
      logger('info', ['getFile', username, studentId, 'log to mongo', result.insertedCount])
    } catch (error) {
