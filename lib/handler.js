...
module.exports.getFile = async (request, response) => {
  ...
  try {
    ...
    if (file) logger('info', ['getFile', username, studentId, `return ${accessType} data`, documentId, recno])
    else logger('warn', ['getFile', username, studentId, `${accessType} data - not found`, documentId, recno])
