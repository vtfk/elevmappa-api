...
const getGroups = async username => {
  ...
  return filteredGroups.map(group => group.displayName.toUpperCase().replace(ACCESS_GROUP_PREFIX, ''))
}
...
module.exports = whitelist => {
  return async (request, response, next) => {
    ...
    if (DEMO) {
      ...
      request.groups = DEMO_ACCESS_GROUPS.map(group => group.replace(ACCESS_GROUP_PREFIX, ''))
      logger('info', ['token-auth', 'username', request.username])
      logger('info', ['token-auth', 'groups', request.groups])
      logger('info', ['token-auth', 'DEMO validated'])
      next()
    } else {
